...
Do note that this is pretty experimental. There is a chance that you'll experience bugs or oddities while using these functions, especially the `encodeModel` function.
...
If you want to index instances deep within your model and without having to do a lot of indexing, you can export the model to code, the second returned value from `encodeModel`.
...
Roblox clamps most strings which are too long. If `print()` and `StringValues` cannot contain the models you're trying to export, you can paste the `encodeModel` function right above the following snippet to get the entire export of whatever instance you select.
```lua
-- V6
...
However, since both options require using scripts, you're out of luck if you need to export exceptionally massive models from a Roblox game rather than your Studio.
